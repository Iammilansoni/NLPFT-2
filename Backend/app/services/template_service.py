...
from app.core.postgres import get_db, AsyncSessionLocal
from app.models.database_models import Template
...
class TemplateService:
                ...
                for template in templates:
                    try:
                        ...
                        result = await session.execute(
                            select(Template).where(Template.api_name == template["intent"])
                        )
    ...
    async def _create_template_db(self, session: AsyncSession, template: Dict):
        ...
        db_template = Template(
            api_name=template.get("api_name", template.get("intent")),
            description=template.get("description", ""),
            base_url=template.get("base_url", ""),
            method=template.get("method", "POST"),
            Field=template.get("type", "api"),
            json_schema=template.get("json_schema", {}),
            sample_requests=template.get("sample_requests", []),
            domain_tags=template.get("domain_tags", []),
            headers=template.get("headers", {}),
            u_id=template.get("user_id") # Ensure user_id is passed
        )
        session.add(db_template)
    
    async def _update_template_db(self, session: AsyncSession, existing: Template, template: Dict):
        """Update existing template in database"""
        existing.api_name = template.get("api_name", template.get("intent"))
        existing.description = template.get("description", "")
        existing.method = template.get("method", "POST")
        existing.json_schema = template.get("json_schema", {})
        existing.sample_requests = template.get("sample_requests", [])
        existing.domain_tags = template.get("domain_tags", [])
        existing.headers = template.get("headers", {})
    ...
    def _db_to_dict(self, db_template: Template) -> Dict:
        """
        ...
        """
        return {
            "id": str(db_template.t_id),
            "intent": db_template.api_name or "",
            "api_name": db_template.api_name or "",
            "description": db_template.description or "",
            "endpoint": "",
            "method": db_template.method or "POST",
            "fields": [],
            "intent_keywords": [],
            "slots_config": {},
            "example_queries": [],
    ...
    async def update_template(self, intent: str, updates: Dict) -> Dict:
        ...
        try:
            async with AsyncSessionLocal() as session:
                result = await session.execute(
                    select(Template).where(Template.api_name == intent)
                )
                existing = result.scalar_one_or_none()
    ...
    async def delete_template(self, intent: str) -> bool:
        ...
        try:
            async with AsyncSessionLocal() as session:
                result = await session.execute(
                    delete(Template).where(Template.api_name == intent)
                )
                await session.commit()
