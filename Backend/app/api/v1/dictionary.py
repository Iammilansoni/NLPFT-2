...
async def list_functions(
    ...
    try:
        ...
        total_count = await service.count_functions(
            category=category,
            is_active=is_active,
            search=search
        )
        
        logger.info(f"Listed {len(functions)} dictionary functions (total: {total_count})")
...
class ImportFunctionsRequest(BaseModel):
    """Request model for importing multiple functions."""
    functions: List[Dict[str, Any]] = Field(..., description="List of functions to import")


@router.post("/import", response_model=Dict[str, Any])
async def import_functions(
    request: ImportFunctionsRequest,
    service: DictionaryService = Depends(get_dictionary_service)
) -> Dict[str, Any]:
    """
    Import multiple dictionary functions.
    
    Bulk import functions from JSON data. Existing functions with the same name will be updated.
    """
    try:
        imported_count = 0
        ...
        for func_data in request.functions:
            try:
                ...
                raw_args = func_data.get('args', func_data.get('arguments', []))
                arguments: List[Dict[str, Any]] = []
                
                for arg in raw_args:
                    if isinstance(arg, str):
                        # Convert string to proper argument object
                        arguments.append({
                            'name': arg,
                            'type': 'str',
                            'required': True,
                        ...
                        # Already a proper argument object - cast to correct type
                        arguments.append(cast(Dict[str, Any], arg))
                
                # Create function from imported data
                await service.create_function(
                    name=func_data.get('function_name', func_data.get('name', '')),
                    templates=func_data.get('templates', []),
                    arguments=arguments,
                ...
                errors.append({
                    'function': func_data.get('function_name', func_data.get('name', 'unknown')),
                    'error': str(e)
                })
        
        logger.info(f"Imported {imported_count} functions with {len(errors)} errors")
        
        return {
            "imported": imported_count,
            "errors": errors,
            "total_attempted": len(request.functions)
        }
        
    except Exception as e:
        logger.error(f"❌ Failed to import functions: {e}")
        raise HTTPException(status_code=500, detail="Failed to import functions")
...
async def trigger_hot_reload(service: DictionaryService = Depends(lambda: get_db_manager().dictionary_service)) -> Dict[str, Any]:
    """
