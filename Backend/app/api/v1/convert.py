...
async def convert(request: ConvertRequest) -> ConvertResponse:
    ...
    try:
        ...
        raw_steps: List[Dict[str, Any]] = engine_result.get("steps", []) or engine_result.get("candidates", []) or []
        unresolved: List[str] = engine_result.get("unresolved_tokens", [])
        ...
        assembled_unresolved: List[str] = assembled.get("unresolved_tokens", [])
        final_unresolved = list(dict.fromkeys((unresolved or []) + assembled_unresolved))
        
        # Convert steps to the expected format for the frontend
        converted_steps: List[Dict[str, Any]] = []
        for step in assembled.get("steps", []):
            converted_step = {
                "function": step.get("function", "unknown"),  # Use 'function' not 'action' for frontend
                "confidence": step.get("confidence", 0.0),
                "args": step.get("args", {}),
                "provenance": step.get("provenance", "template_match"),
                "template": step.get("template", ""),
                "matched_text": step.get("matched_text", "")
            }
        ...
        # Return Enhanced Rule Engine response in the format expected by frontend
        return ConvertResponse(
            original_text=request.text,
            converted_text=json.dumps(result_dict, indent=2),
            ...
            metadata={
                ...
                # Add the actual data the frontend expects
                "steps": converted_steps,
                "overall_confidence": overall_confidence,
                "unresolved_tokens": final_unresolved,
