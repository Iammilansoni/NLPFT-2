...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[60vh] flex-col items-center justify-center overflow-hidden bg-background w-full rounded-md z-0 pt-32",
        className
      )}
    >
      <div className="relative flex w-full flex-1 scale-y-125 scale-x-75 sm:scale-x-100 items-center justify-center isolate z-0 ">
        <motion.div
        ...
        <div className="absolute inset-auto z-40 h-36 w-[28rem] -translate-y-1/2 rounded-full bg-primary opacity-50 blur-3xl"></div>
        <motion.div
          initial={{ width: "8rem" }}
        ...
        <motion.div
          ...
          className="absolute inset-auto z-40 h-0.5 w-[30rem] -translate-y-[7rem] bg-primary/80"
        ></motion.div>

        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-[12.5rem] bg-background"></div>
      </div>

      <div className="relative z-50 flex -translate-y-12 sm:-translate-y-24 flex-col items-center px-5 w-full">
        {children}
      </div>
    </div>
  );
